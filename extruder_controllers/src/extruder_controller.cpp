...
#include "extruder_controller/extruder_rule_impl.hpp"
...
{
controller_interface::CallbackReturn ExtruderController::on_init()
{
  // initialize controller config
  try
  {
    parameter_handler_ = std::make_shared<extruder_controller::ParamListener>(get_node());
    extruder_ = std::make_unique<extruder_controller::ExtruderRule>(parameter_handler_);
  }
  ...
  num_joints_ = extruder_->parameters_.joints.size();
  ...
  reference_extruder_ = last_reference_;
  joint_state_ = last_reference_;

  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::InterfaceConfiguration ExtruderController::command_interface_configuration()
  const
{
  std::vector<std::string> command_interfaces_config_names;
  for (const auto & interface : extruder_->parameters_.command_interfaces)
  {
...
controller_interface::InterfaceConfiguration ExtruderController::state_interface_configuration()
  const
{
  std::vector<std::string> state_interfaces_config_names;
  for (size_t i = 0; i < extruder_->parameters_.state_interfaces.size(); ++i)
  {
    const auto & interface = extruder_->parameters_.state_interfaces[i];
    for (const auto & joint : extruder_->parameters_.joints)
    {
      auto full_name = joint + "/" + interface;
...
ExtruderController::on_export_reference_interfaces()
{
  ...
  const auto num_chainable_interfaces =
    extruder_->parameters_.chainable_command_interfaces.size() *
    extruder_->parameters_.joints.size();
...
controller_interface::CallbackReturn ExtruderController::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  command_joint_names_ = extruder_->parameters_.command_joints;
  if (command_joint_names_.empty())
  {
    command_joint_names_ = extruder_->parameters_.joints;
    RCLCPP_INFO(
      get_node()->get_logger(),
  ...
  for (const auto & tmp : extruder_->parameters_.state_interfaces)
  {
    RCLCPP_INFO(get_node()->get_logger(), "%s", ("state int types are: " + tmp + "\n").c_str());
  }
  for (const auto & tmp : extruder_->parameters_.command_interfaces)
  {
    RCLCPP_INFO(get_node()->get_logger(), "%s", ("command int types are: " + tmp + "\n").c_str());
  }

  // validate exported interfaces
  for (const auto & tmp : extruder_->parameters_.chainable_command_interfaces)
  {
  ...
  has_position_command_interface_ = contains_interface_type(
    extruder_->parameters_.command_interfaces, hardware_interface::HW_IF_POSITION);
  has_velocity_command_interface_ = contains_interface_type(
    extruder_->parameters_.command_interfaces, hardware_interface::HW_IF_VELOCITY);
  has_acceleration_command_interface_ = contains_interface_type(
    extruder_->parameters_.command_interfaces, hardware_interface::HW_IF_ACCELERATION);
  has_effort_command_interface_ = contains_interface_type(
    extruder_->parameters_.command_interfaces, hardware_interface::HW_IF_EFFORT);

  // Check if only allowed interface types are used and initialize storage to avoid memory
  // allocation during activation
  joint_state_interface_.resize(allowed_interface_types_.size());
  for (const auto & interface : extruder_->parameters_.state_interfaces)
  {
    auto it =
      std::find(allowed_interface_types_.begin(), allowed_interface_types_.end(), interface);
    if (it == allowed_interface_types_.end())
  ...
  has_position_state_interface_ = contains_interface_type(
    extruder_->parameters_.state_interfaces, hardware_interface::HW_IF_POSITION);
  has_velocity_state_interface_ = contains_interface_type(
    extruder_->parameters_.state_interfaces, hardware_interface::HW_IF_VELOCITY);
  has_acceleration_state_interface_ = contains_interface_type(
    extruder_->parameters_.state_interfaces, hardware_interface::HW_IF_ACCELERATION);
  ...
  RCLCPP_INFO(
    ...
    get_interface_list(extruder_->parameters_.command_interfaces).c_str(),
    get_interface_list(extruder_->parameters_.state_interfaces).c_str());
  ...
  s_publisher_ = get_node()->create_publisher<control_msgs::msg::ExtruderControllerState>(
  ...
  state_publisher_->msg_ = extruder_->get_controller_state();
  ...
  if (extruder_->configure(get_node(), num_joints_) == controller_interface::return_type::ERROR)
  {
    return controller_interface::CallbackReturn::ERROR;
  }
...
controller_interface::CallbackReturn ExtruderController::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  for (const auto & interface : extruder_->parameters_.state_interfaces)
  {
    ...
    if (!controller_interface::get_ordered_interfaces(
          state_interfaces_, extruder_->parameters_.joints, interface,
  ...
  extruder_->apply_parameters_update();
  ...
  reference_extruder_ = joint_state_; //Attention

  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::return_type ExtruderController::update_reference_from_subscribers(
...
controller_interface::return_type ExtruderController::update_and_write_commands(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // apply extruder control to reference to determine desired state
  extruder_->update(joint_state_, ft_values_, reference_, period, reference_extruder_); // attention

  // write calculated values to joint interfaces
  write_state_to_hardware(reference_extruder_); // attention
...
controller_interface::CallbackReturn ExtruderController::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  if (!extruder_)
  {
  ...
  release_interfaces();
  extruder_->reset(num_joints_);

  return CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn ExtruderController::on_cleanup(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn ExtruderController::on_error(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  if (!extruder_)
  {
...
void ExtruderController::read_state_from_hardware(
...
void ExtruderController::write_state_to_hardware(
...
void ExtruderController::read_state_reference_interfaces(
